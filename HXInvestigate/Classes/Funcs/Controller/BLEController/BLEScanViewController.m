...
-(void)babyDelegate{
    ...
    [baby setBlockOnDiscoverToPeripherals:^(CBCentralManager *central, CBPeripheral *peripheral, NSDictionary *advertisementData, NSNumber *RSSI) {
        NSLog(@"搜索到了设备:%@ : %@",peripheral.name,peripheral.identifier);
    ...
    [baby setFilterOnDiscoverPeripherals:^BOOL(NSString *peripheralName, NSDictionary *advertisementData, NSNumber *RSSI) {
        ...
        //最常用的场景是查找某一个前缀开头的设备
        if ([peripheralName rangeOfString:@"Smart"].length > 0) {
            return YES;
        }
        return NO;
    }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([advertisementData objectForKey:@"kCBAdvDataLocalName"]) {
        peripheralName = [NSString stringWithFormat:@"%@:%@",peripheral.name,peripheral.identifier];//[NSString stringWithFormat:@"%@",[advertisementData objectForKey:@"kCBAdvDataLocalName"]];
    }else if(!([peripheral.name isEqualToString:@""] || peripheral.name == nil)){
        peripheralName = [NSString stringWithFormat:@"%@:%@",peripheral.name,peripheral.identifier];
    }else{
