...
@interface BSBlurView ()

@property (nonatomic, strong) BSGradientView *gradientBackgroundView;
...
- (BSGradientView *)gradientBackgroundView {
    if (!_gradientBackgroundView) {
        _gradientBackgroundView = [[BSGradientView alloc] initWithFrame:self.bounds];
    }
...
- (UIView *)backgroundView {
    switch (self.blurStyle) {
        case BSBlurStyleBlackGradient:
        return self.gradientBackgroundView;
        break;
        case BSBlurStyleTranslucent:
        return self.blurBackgroundView;
        case BSBlurStyleBlackTranslucent:
        return self.blackTranslucentBackgroundView;
        break;
        case BSBlurStyleWhite:
...
- (void)setup {
    ...
    self.blurStyle = BSBlurStyleTranslucent;
    self.backgroundColor = [UIColor clearColor];
...
#pragma mark - UIView BSBlurView分类的实现

@implementation UIView (BSBlurView)

#pragma mark - Show Block

- (WillShowBlurViewBlcok)willShowBlurViewcomplted {
    return objc_getAssociatedObject(self, &BSRealTimeWillShowBlurViewBlcokBlcokKey);
}

- (void)setWillShowBlurViewcomplted:(WillShowBlurViewBlcok)willShowBlurViewcomplted {
    objc_setAssociatedObject(self, &BSRealTimeWillShowBlurViewBlcokBlcokKey, willShowBlurViewcomplted, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

- (DidShowBlurViewBlcok)didShowBlurViewcompleted {
    return objc_getAssociatedObject(self, &BSRealTimeDidShowBlurViewBlcokBlcokKey);
}

- (void)setDidShowBlurViewcompleted:(DidShowBlurViewBlcok)didShowBlurViewcompleted {
    objc_setAssociatedObject(self, &BSRealTimeDidShowBlurViewBlcokBlcokKey, didShowBlurViewcompleted, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

#pragma mark - dismiss block

- (WillDismissBlurViewBlcok)willDismissBlurViewCompleted {
    return objc_getAssociatedObject(self, &BSRealTimeWillDismissBlurViewBlcokKey);
}

- (void)setWillDismissBlurViewCompleted:(WillDismissBlurViewBlcok)willDismissBlurViewCompleted {
    objc_setAssociatedObject(self, &BSRealTimeWillDismissBlurViewBlcokKey, willDismissBlurViewCompleted, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

- (DidDismissBlurViewBlcok)didDismissBlurViewCompleted {
    return objc_getAssociatedObject(self, &BSRealTimeDidDismissBlurViewBlcokKey);
}

- (void)setDidDismissBlurViewCompleted:(DidDismissBlurViewBlcok)didDismissBlurViewCompleted {
    objc_setAssociatedObject(self, &BSRealTimeDidDismissBlurViewBlcokKey, didDismissBlurViewCompleted, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

#pragma mark - RealTimeBlur HUD


- (BSBlurView *)realTimeBlur {
    return objc_getAssociatedObject(self, &BSBlurViewKey);
}

- (void)setRealTimeBlur:(BSBlurView *)realTimeBlur {
    objc_setAssociatedObject(self, &BSBlurViewKey, realTimeBlur, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)showRealTimeBlurWithBlurStyle:(BSBlurStyle)blurStyle hasTapGestureEnable:(BOOL)hasTapGestureEnable {
    BSBlurView *realTimeBlur = [self realTimeBlur];
    if (!realTimeBlur) {
        realTimeBlur = [[BSBlurView alloc] initWithFrame:self.bounds];
