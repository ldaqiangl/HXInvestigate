...
const CGFloat kLineSpacing = 1.0;  // 如果间距不一样 会影响点击时背景的 不对。
...
/**
 *  TTT、。commonInit是被调用了两回。如果直接init的话，因为init其中会调用initWithFrame
 *  PS.已经在里面把init里的修改掉了
 */
- (void)commonInit {
...
#pragma  这里可以修改电话号码   @   #   网址  的颜色
    UIColor *commonLinkColor =[UIColor redColor];
//    [UIColor colorWithRed:0.112 green:0.000 blue:0.791 alpha:1.000];
  
    
#pragma 这里可以修改点击时候的背景色
    //点击时候的背景色
...
- (NSMutableAttributedString*)mutableAttributeStringWithEmojiText:(NSString*)emojiText
{
    ...
    for (NSTextCheckingResult *result in emojis) {
        ...
        NSDictionary *emojiDict = self.customEmojiRegularExpression?self.customEmojiDictionary:[MxlLabel emojiDictionary];
