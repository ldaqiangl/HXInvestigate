...
- (id)initWithFrame:(CGRect)frame byClick:(NSInteger)clickTag appendArray:(NSArray *)appendArray{
    ...
    if (self) {
        ...
        _label.frame = CGRectMake(0, self.frame.size.height - 40, self.frame.size.width,30);
        _label.textAlignment = NSTextAlignmentCenter;
        _label.font = [UIFont systemFontOfSize:16];
        _label.backgroundColor = [UIColor clearColor];
        _label.textColor = [UIColor whiteColor];
        [self addSubview:_label];
        _label.text = [NSString stringWithFormat:@"%ld/%ld",(clickTag+1),(unsigned long)appendArray.count];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if (scollV_pre.zoomScale != 1.0){
        ...
        _label.text = [NSString stringWithFormat:@"%ld/%ld",page + 1,_appendArray.count];
