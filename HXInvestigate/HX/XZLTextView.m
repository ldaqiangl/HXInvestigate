...
@interface XZLTextView ()<UITextViewDelegate,UIScrollViewDelegate>
{
...
- (void)createUI{
    ...
    [self.textView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(PX_TO_PT(10));
        make.left.mas_equalTo(12);
        make.bottom.mas_equalTo(-6);
        make.width.mas_equalTo(kScreenwidth-PX_TO_PT(80));
    }];
    ...
    [self.sendButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(PX_TO_PT(10));
        make.right.mas_equalTo(-PX_TO_PT(12));
        make.width.mas_equalTo(PX_TO_PT(44));
    }];
...
- (void)textViewDidChange:(UITextView *)textView{
    ...
    if (curheight < 19.094) {
        statusTextView = NO;
        self.backGroundView.frame = CGRectMake(0, y - PX_TO_PT(49), kScreenwidth, PX_TO_PT(49));
...
- (void)sendClick:(UIButton *)sender{
    ...
    if (self.XZLTextViewBlock) {
        self.XZLTextViewBlock(self.textView.text);
    }
    ...
    [self.sendButton setBackgroundColor:UIColorRGB(180, 180, 180)];
    self.sendButton.userInteractionEnabled = NO;
    self.frame = CGRectMake(0, kScreenheight-PX_TO_PT(49)-64, kScreenwidth, PX_TO_PT(49));
    self.backGroundView.frame = CGRectMake(0, 0, kScreenwidth, PX_TO_PT(49));
}
...
- (UIView *)backGroundView{
    if (!_backGroundView) {
        _backGroundView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kScreenwidth, PX_TO_PT(49))];
        _backGroundView.backgroundColor = [UIColor colorWithHexString_Ext:FColor];
        _backGroundView.backgroundColor = [UIColor redColor];
        [self addSubview:_backGroundView];
        
        UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenwidth, PX_TO_PT(1))];
        lineView.backgroundColor = [UIColor colorWithHexString_Ext:@"#E5E5E5"];
...
- (UITextView *)textView{
    if (!_textView) {
        ...
        _textView.font = [UIFont systemFontOfSize:PX_TO_PT(16)];
        _textView.delegate = self;
        _textView.layer.cornerRadius = PX_TO_PT(8);
        _textView.layer.borderWidth = PX_TO_PT(1);
...
//        [_sendButton setBackgroundColor:UIColorRGB(180, 180, 180)];
        ...
        _sendButton.layer.cornerRadius = PX_TO_PT(3);
